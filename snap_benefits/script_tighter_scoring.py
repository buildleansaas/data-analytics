...
"""
Tighter, more conservative variant of per‑capita SNAP participation by detailed ancestry
using ACS PUMS 2023 1‑year.

Conservative choices:
  - Excludes Group Quarters (GQ) entirely; counts Housing Units (HU) only.
  - Uses primary detailed ancestry only (ANC1P); no fallback to ANC2P.
  - Treats missing household FS as non‑SNAP during the person–household join.

Inputs (unzipped CSVs from Census "csv_hus.zip" and "csv_pus.zip"):
  - psam_husa.csv, psam_husb.csv  (HOUSEHOLD files; need SERIALNO, FS)
  - psam_pusa.csv, psam_pusb.csv  (PERSON files; need SERIALNO, PWGTP, ANC1P)

Outputs:
  - out/snap_per_capita_ancestry_tighter.csv  (ancestry_label|people|people_in_snap_hh|rate)
  - out/snap_per_capita_tighter_topN.png      (ranked bar chart, highest per-capita rates)
  - out/snap_per_capita_tighter_all.png       (all groups)
"""
...
def parse_args():
    ap = argparse.ArgumentParser(description="Conservative per-capita SNAP by ancestry (HU only, ANC1P only).")
...
P_USECOLS = ["SERIALNO", "PWGTP", "ANC1P"]


def is_hu_serial(series: pd.Series) -> pd.Series:
    s = series.astype("string")
    return s.str.slice(4, 6) == "HU"
...
def load_household_snap(paths: List[str], chunksize: int) -> pd.Series:
    ...
    for p in paths:
        for chunk in pd.read_csv(p, usecols=H_USECOLS, chunksize=chunksize,
            ...
            # HU only
            mask_hu = is_hu_serial(chunk["SERIALNO"])  # True for HU, False for GQ
            snap = (chunk.loc[mask_hu, "FS"] == 1)
            snap.index = chunk.loc[mask_hu, "SERIALNO"].astype("string")
            acc.append(snap)
    if not acc:
...
def ancestry_code_primary(df: pd.DataFrame) -> np.ndarray:
    # Use only ANC1P; if >0 keep; else assign -1
    a1 = df["ANC1P"].fillna(0).astype("Int64")
    code = a1.where(a1 > 0, -1)
...
def aggregate_people_by_ancestry(person_paths: List[str],
    ...
    for p in person_paths:
        for chunk in pd.read_csv(p, usecols=P_USECOLS, chunksize=chunksize,
                                 dtype={"SERIALNO": "string", "PWGTP": "float64", "ANC1P": "Int64"}):
            # HU only persons
            mask_hu = is_hu_serial(chunk["SERIALNO"])  # filter out GQ
            if not mask_hu.any():
                continue
            sub = chunk.loc[mask_hu]

            # Join HU-only household SNAP status by SERIALNO
            snap = (hh_snap.reindex(sub["SERIALNO"].astype("string"))
                    .fillna(False).astype(bool).to_numpy())
            anc = ancestry_code_primary(sub)
            w = sub["PWGTP"].to_numpy()
...
def main():
    ...
    # HU-only household SNAP indicator
    hh_snap = load_household_snap(args.household, args.chunksize)

    # Aggregate person-weighted totals by primary ancestry (ANC1P), HU persons only
    ...
    out_csv = os.path.join(args.outdir, "snap_per_capita_ancestry_tighter.csv")
    res.sort_values("rate", ascending=False).to_csv(out_csv, index=False)

    plot_bar(res, os.path.join(args.outdir, "snap_per_capita_tighter_topN.png"),
             "Per-capita SNAP (conservative; HU only; ANC1P only) — Top N", args.topn)
    plot_bar(res, os.path.join(args.outdir, "snap_per_capita_tighter_all.png"),
             "Per-capita SNAP (conservative; HU only; ANC1P only) — All", topn=0)
